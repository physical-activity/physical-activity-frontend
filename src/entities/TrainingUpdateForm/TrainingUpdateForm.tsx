...
export const TrainingUpdateForm = ({ id, card }: Props) => {
	type TypeItem = {
		name: string;
	};
	...
	function setValues() {
		let date: string = `${new Intl.DateTimeFormat('en-GB').format(
			new Date(card.started_at)
		)}`;
		let startedAtTime: string = formatterTime.format(new Date(card.started_at));
		let finishedAtTime: string = formatterTime.format(
			new Date(card.finished_at)
		);
		setTrainingTypeInputValue(card.training_type);
		setTrainingDateInputValue(date);
		setTrainingStartedAtInputValue(startedAtTime);
		setTrainingFinishedAtInputValue(finishedAtTime);
		setTrainingDistanceInputValue(card.distance);
		setTrainingStepsNumInputValue(card.steps_num);
		setIsTrainingReminder(card.reminder);
	}
